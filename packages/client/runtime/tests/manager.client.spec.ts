...
describe('resident window pruning', () => {
  const ids = (count: number): SessionId[] =>
    Array.from({ length: count }, (_, i) => `fk-prune-${i}` as SessionId)

  it('releases the oldest non-selected windows past the cap on select', async () => {
    const many = ids(25)
    const api = new FakeApiClient()
    api.onList = () => Promise.resolve(ok({ items: many.map(id => summary(id)) as never[] }))
    const manager = new SessionManager(api, fakeRemote())
    await manager.refreshList()
    const sessions = many.map(id => manager.get(id))
    await Promise.all(sessions.map(session => session.open()))
    for (let i = 0; i < many.length; i++) sessions[i]!.lastActiveAt = i * 1000
    manager.select(many[5]!)
    // 25 windows over the cap of 20: the five oldest are released, the rest stay.
    for (let i = 0; i < 5; i++) expect(sessions[i]!.hasOpenWindow()).toBe(false)
    for (let i = 5; i < many.length; i++) expect(sessions[i]!.hasOpenWindow()).toBe(true)
    // The released instance stays warm: opening again re-fetches.
    await sessions[0]!.open()
    expect(sessions[0]!.hasOpenWindow()).toBe(true)
  })

  it('spares the running session and the selected session', async () => {
    const many = ids(25)
    const api = new FakeApiClient()
    api.onList = () => Promise.resolve(ok({
      items: many.map((id, i) => summary(id, { running: i === 0 })) as never[],
    }))
    const manager = new SessionManager(api, fakeRemote())
    await manager.refreshList()
    const sessions = many.map(id => manager.get(id))
    await Promise.all(sessions.map(session => session.open()))
    for (let i = 0; i < many.length; i++) sessions[i]!.lastActiveAt = i * 1000
    manager.select(many[7]!)
    // Running session 0 and selected session 7 survive; the oldest five of the
    // remaining candidates (1..6, 8..24 by lastActiveAt) are released: 1..5.
    expect(sessions[0]!.hasOpenWindow()).toBe(true)
    expect(sessions[7]!.hasOpenWindow()).toBe(true)
    expect(sessions[1]!.hasOpenWindow()).toBe(false)
    expect(sessions[5]!.hasOpenWindow()).toBe(false)
    expect(sessions[6]!.hasOpenWindow()).toBe(true)
    expect(sessions[8]!.hasOpenWindow()).toBe(true)
  })
})
