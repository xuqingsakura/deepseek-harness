...
- **Confined children run windowless.** The runner creates confined children with `CREATE_NO_WINDOW`, so a GUI host never flashes a console window per command. The POC-era `STATUS_DLL_INIT_FAILED` (`0xC0000142`) observation only occurred while the console logon SID (`S-1-2-1`) sat in the restricting list; the shipped lists omit it (on Windows 11 26200 `CreateWellKnownSid(WinLocalLogonSid)` fails with `ERROR_INVALID_PARAMETER` (87)), and with it absent `CREATE_NO_WINDOW` is verified usable. stdio redirection is pipe-based and unaffected.
