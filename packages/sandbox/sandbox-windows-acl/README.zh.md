...
- **受限子进程无窗口运行。** runner 以 `CREATE_NO_WINDOW` 创建受限子进程，GUI 宿主不会为每条命令闪现控制台窗口。POC 时代的 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）现象只发生在 restricting 列表含控制台登录 SID（`S-1-2-1`）时；当前发布的列表不含它（Windows 11 26200 上 `CreateWellKnownSid(WinLocalLogonSid)` 以 `ERROR_INVALID_PARAMETER`（87）失败），去掉它之后 `CREATE_NO_WINDOW` 经验证可用。stdio 重定向走管道，不受影响。
